...
static void dbproxy_exit_cb(uv_signal_t *handle, int num) {
  if (dbproxy_server) dbproxy_server->Shutdown();
}
...
std::tuple<string, string, string, string, string> ParseMySQLURL(
    const string &url);
...
class DBTimerListener : public uv::TimerListener {
 public:
  ...
  virtual void OnTimeout(uv::Timer *timer) { server_->OnTimer(); }
...
void DBProxy::ShowStat(void) {
  ...
  for (int i = 0; i < kDBStatNR; i++) {
    auto st = &dbstat_[i];
    n +=
        sprintf(buf + n, "%d\t%d\t%d\t%d\t%d\t%d\n", st->delay[0], st->delay[1],
                st->delay[2], st->delay[3], st->delay[4], st->delay[5]);
  }
...
int DBProxy::InitDBClients(LuaRef &db, void *loop) {
  ...
  for (int i = 0; i < dbclient_count_; i++) {
    ...
    dbclients_[i]->Init(std::bind(&DBProxy::AfterDB, this, _1, _2, _3), loop,
                        mysql_url);
...
int DBProxy::InitRedisClients(LuaRef &db, void *loop) {
  ...
  for (int i = 0; i < rdclient_count_; i++) {
    ...
    rdclients_[i]->Init(std::bind(&DBProxy::AfterCache, this, _1, _2, _3, _4),
                        loop, redis_url);
  }
...
Handle *DBProxy::FindHandleSeq(uint32_t seq) {
  ...
  if (it == handlemap_.end()) return NULL;
  return it->second;
}
...
int DBProxy::CacheSet(Handle *h, const char *key, const char *buf,
                      size_t bufsize) {
  uint32_t seq = nextseq();
...
int DBProxy::Reply(Handle *handle, const char *src, size_t len) {
  ...
  h->retcode = 0;
  if (len > 0) memcpy(buf + sizeof(RespHeader), src, len);
...
int DBProxy::SendMsg(Handle *handle, Message *msg) {
  ...
  if (msg) {
    bool succ =
        msg->SerializeToArray(buf + sizeof(Header), len - sizeof(Header));
    if (!succ) {
      LogError("SerializeToArray failed\n");
      DelTCPWriteRequest(req);
      return -1;
...
int DBProxy::DBSet(Handle *h, const std::string &msg_type,
  ...
  uint32_t seq = nextseq();
  int n = h->dbclient->Set(seq, msg_type, value);
  if (n == 0) AddHandleSeq(seq, h);
  return n;
}
...
int DBProxy::DBSetNX(Handle *h, const std::string &msg_type,
                     const std::string &value) {
  uint32_t seq = nextseq();
  int n = h->dbclient->SetNX(seq, msg_type, value);
  if (n == 0) AddHandleSeq(seq, h);
  return n;
}
...
void DBProxy::Shutdown() {
  if (conn_count_ > 0) {
    LogError(
        "stopping dbproxy but client connection count is %zd, waiting...\n",
        conn_count_);
    closing_ = true;
    return;
  }
...
int DBProxy::InitFromArgs(int argc, char **argv) {
  ...
  common::ImmortalLog::Init(log_level, log_path_.c_str(), log_file_size,
                            log_file_num, print);
